...
<template>
  <div>
    <div class="flex items-center gap-3 mb-6">
      ...
      <span class="text-[var(--ui-text-muted)]"> ({{ books?.length ?? 0 }}) </span>
    </div>
    ...
    <section
      ...
      class="mb-8"
    >
      <h2 class="text-lg font-medium mb-4 text-[var(--ui-text-muted)]">Currently Reading</h2>
      <ContentList :items="currentlyReading" />
    </section>

    <!-- Want to Read -->
    <section
      v-if="wantToRead.length"
      class="mb-8"
    >
      <h2 class="text-lg font-medium mb-4 text-[var(--ui-text-muted)]">Want to Read</h2>
      <ContentList :items="wantToRead" />
    </section>

    <!-- Books by Year -->
    <section
      v-for="{ year, books: yearBooks } in byYear"
      :key="year"
      class="mb-8"
    >
