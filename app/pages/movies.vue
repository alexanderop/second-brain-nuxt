...
const { data: movies } = await useAsyncData('all-movies', () => {
  return queryCollection('content').where('type', '=', 'movie').order('watchedOn', 'DESC').all();
});
...
<template>
  <div>
    <div class="flex items-center gap-3 mb-6">
      ...
      <span class="text-[var(--ui-text-muted)]"> ({{ movies?.length ?? 0 }}) </span>
    </div>

    <!-- Currently Watching -->
    <section
      v-if="currentlyWatching.length"
      class="mb-8"
    >
      <h2 class="text-lg font-medium mb-4 text-[var(--ui-text-muted)]">Currently Watching</h2>
      <ContentList :items="currentlyWatching" />
    </section>

    <!-- Want to Watch -->
    <section
      v-if="wantToWatch.length"
      class="mb-8"
    >
      <h2 class="text-lg font-medium mb-4 text-[var(--ui-text-muted)]">Want to Watch</h2>
      <ContentList :items="wantToWatch" />
    </section>

    <!-- Movies by Year -->
    <section
      v-for="{ year, movies: yearMovies } in byYear"
      :key="year"
      class="mb-8"
    >
    ...
    <section
      v-if="noStatus.length"
      class="mb-8"
    >
      <h2 class="text-lg font-medium mb-4 text-[var(--ui-text-muted)]">Untracked</h2>
