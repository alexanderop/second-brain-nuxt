...
export function applyFiltersExcept<
  T extends { type: ContentType; tags?: string[]; authors?: TableAuthor[] },
>(items: T[], filters: FilterState, exclude: keyof FilterState): T[] {
  let result = items;
...
export function buildAuthorMap(
  authors: Array<{ slug: string; name: string; avatar?: string }>,
): Map<string, TableAuthor> {
 ...
 */
export function enrichContentWithAuthors<
  T extends {
    stem: unknown;
    title: string;
    type: ContentType;
...
>(content: T[], authorMap: Map<string, TableAuthor>): TableContentItem[] {
