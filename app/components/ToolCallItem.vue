    ...
    >
      ...
      >
        ...
        <UIcon
          ...
          class="shrink-0"
          aria-hidden="true"
          :class="[isPending ? 'animate-spin text-[var(--ui-text-muted)]' : 'text-green-500']"
        />
        <UIcon
      ...
      <div v-if="isExpanded" class="px-3 py-2 border-t border-[var(--ui-border)] text-sm">
        <div class="mb-2">
          ...
          <pre class="text-xs bg-[var(--ui-bg-muted)] p-2 rounded overflow-x-auto">{{
            formatJson(input)
          }}</pre>
        </div>
        <div v-if="result">
          ...
          <pre
            class="text-xs bg-[var(--ui-bg-muted)] p-2 rounded overflow-x-auto max-h-48 overflow-y-auto"
            >{{ formatJson(result) }}</pre
          >
        </div>
        <div v-else class="text-xs text-[var(--ui-text-muted)] italic">Waiting for result...</div>
      </div>
    </div>
  </div>
</template>
