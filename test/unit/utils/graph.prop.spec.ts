...
describe('createNode (property-based)', () => {
  const contentItemArb: fc.Arbitrary<ContentItem> = fc.record({
    ...
    tags: fc.option(fc.oneof(fc.array(fc.string(), { maxLength: 5 }), fc.constant(undefined)), {
      nil: undefined,
    }),
    authors: fc.option(fc.oneof(fc.array(fc.string(), { maxLength: 3 }), fc.constant(undefined)), {
      nil: undefined,
    }),
      ...
      fc.property(
        ...
        (nodeIds) => {
          ...
          const edges: GraphEdge[] =
            first !== undefined && second !== undefined ? [{ source: first, target: second }] : [];
