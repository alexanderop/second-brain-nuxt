...
describe("transformWikiLink", () => {
  ...
  it("transforms slug with display text", () => {
    expect(transformWikiLink("slug", undefined, "Display Text")).toBe(
      "[Display Text](/slug){.wiki-link}",
    );
  });
  ...
  it("handles slug with multiple words and custom display", () => {
    expect(transformWikiLink("Local First Software", undefined, "local-first")).toBe(
      "[local-first](/local-first-software){.wiki-link}",
    );
  });

  it("transforms slug with heading", () => {
    expect(transformWikiLink("slug", "Key Insights")).toBe(
      "[slug#Key Insights](/slug#key-insights){.wiki-link}",
    );
  });

  it("transforms slug with heading and display text", () => {
    expect(transformWikiLink("slug", "heading", "Custom Text")).toBe(
      "[Custom Text](/slug#heading){.wiki-link}",
    );
  });

  it("normalizes heading in URL but preserves in display", () => {
    expect(transformWikiLink("atomic-habits", "Key Insights")).toBe(
      "[atomic-habits#Key Insights](/atomic-habits#key-insights){.wiki-link}",
    );
  });
  ...
  it("trims whitespace from slug and heading in display", () => {
    expect(transformWikiLink("  slug  ", "  heading  ")).toBe(
      "[slug#heading](/slug#heading){.wiki-link}",
    );
  });
...
describe("transformWikiLinks", () => {
  it("transforms single wiki-link", () => {
    expect(transformWikiLinks("See [[note]]")).toBe("See [note](/note){.wiki-link}");
  });

  it("transforms wiki-link with display text", () => {
    expect(transformWikiLinks("Read [[my-article|this article]]")).toBe(
      "Read [this article](/my-article){.wiki-link}",
    );
  });

  it("transforms multiple wiki-links", () => {
    expect(transformWikiLinks("See [[foo]] and [[bar]]")).toBe(
      "See [foo](/foo){.wiki-link} and [bar](/bar){.wiki-link}",
    );
  });
...
Regular [markdown](http://example.com) links stay intact.`;
  ...
  it("handles wiki-links with spaces in slug", () => {
    expect(transformWikiLinks("See [[My Cool Note]]")).toBe(
      "See [My Cool Note](/my-cool-note){.wiki-link}",
    );
  });

  it("handles consecutive wiki-links", () => {
    expect(transformWikiLinks("[[foo]][[bar]]")).toBe(
      "[foo](/foo){.wiki-link}[bar](/bar){.wiki-link}",
    );
  });

  it("handles wiki-link at start of text", () => {
    expect(transformWikiLinks("[[note]] is referenced")).toBe(
      "[note](/note){.wiki-link} is referenced",
    );
  });
  ...
  it("transforms wiki-link with heading", () => {
    expect(transformWikiLinks("See [[note#section]]")).toBe(
      "See [note#section](/note#section){.wiki-link}",
    );
  });

  it("transforms wiki-link with heading and display text", () => {
    expect(transformWikiLinks("See [[note#section|the section]]")).toBe(
      "See [the section](/note#section){.wiki-link}",
    );
  });

  it("transforms wiki-link with heading containing spaces", () => {
    expect(transformWikiLinks("See [[atomic-habits#Key Insights]]")).toBe(
      "See [atomic-habits#Key Insights](/atomic-habits#key-insights){.wiki-link}",
    );
  });

  it("handles mixed wiki-links with and without headings", () => {
    expect(transformWikiLinks("See [[foo]] and [[bar#section]]")).toBe(
      "See [foo](/foo){.wiki-link} and [bar#section](/bar#section){.wiki-link}",
    );
  });
});
