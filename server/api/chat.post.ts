...
async function executeSearchNotes(
  ...
  log.info(
    `[${requestId}] keyword search found ${notes.length} results:`,
    notes.map((n) => n.title),
  );
  return notes;
}
...
async function executeFetchSource(
  ...
  if (fetchError) {
    log.error(`[${requestId}] fetch_source: failed to fetch URL`, {
      url: note.url,
...
async function executeTool(
  ...
  if (toolName === "get_note_details" && isGetNoteDetailsInput(toolInput)) {
    return handleGetNoteDetails(
      httpEvent,
      toolInput.slug,
      toolInput.include_related ?? true,
      requestId,
    );
  }
    ...
    while (continueLoop) {
      ...
      if (response.stop_reason === "tool_use") {
        ...
        for (const toolUse of toolUseBlocks) {
          ...
          const { result, notes } = await executeTool(
            httpEvent,
            toolUse.name,
            toolUse.input,
            requestId,
