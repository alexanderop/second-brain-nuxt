...
function createOgImageMarkup(title: string, description: string, type: string): SatoriElement {
...
async function main() {
  ...
  for (const filePath of files) {
    ...
    const stats = await Promise.all([stat(filePath), stat(outputPath).catch(() => null)]);
