...
import { createNoteGraphData, createFullGraphData } from "../factories/graphFactory";
  ...
  describe("slots", () => {
    ...
    it("renders header slot with counts", async () => {
      render(BaseGraph, {
        ...
        slots: {
          header:
            '<template #header="{ nodeCount, edgeCount }"><div data-testid="header">{{ nodeCount }} nodes</div></template>',
        },
      });
